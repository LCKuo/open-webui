    ...
    if event_emitter:
        ...
        async def response_handler(response, events):
            ...
            try:
                ...
                canonical_content = output_text(output).strip() or ''.join(content_parts).strip()
                title = await Chats.get_chat_title_by_id(metadata['chat_id']) if save_to_chat else ''
                data = {
                    'done': True,
                ...
                await publish_chat_finished_event(request, user, metadata, title, canonical_content, output)
