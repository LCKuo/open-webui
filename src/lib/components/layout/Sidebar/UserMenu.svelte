<script lang="ts">
	...
	import { companyWallet, showSettings, mobile, showSidebar, user, config, settings } from '$lib/stores';
	...
	const getBillingWalletInfo = async () => {
		...
		if (res) {
			companyWallet.set({ enabled: res.enabled, wallet: res.wallet });
		}
	};
			...
			{#if $user}
				...
				{#if $companyWallet?.enabled && $companyWallet?.wallet}
					<a
						...
						target="_blank"
						rel="noreferrer"
						class="mx-1 mb-1 flex items-center justify-between gap-3 rounded-xl bg-gray-50 px-2 py-2 text-xs text-black transition dark:bg-gray-900/50 dark:text-white"
					>
			...
			{#if ($config?.features?.enable_workflows ?? true) && ($user?.role === 'admin' || ($user?.permissions?.features?.workflows ?? true))}
				<div class="flex w-full items-center">
					<a
						href="/workflows"
						draggable="false"
						class="flex h-[1.6875rem] flex-1 items-center gap-2 rounded-xl px-2 text-[13px] transition hover:bg-gray-50/40 dark:hover:bg-gray-800/40"
						on:click={async (e) => {
							if (e.metaKey || e.ctrlKey || e.shiftKey || e.button === 1) return;
							e.preventDefault();
							show = false;
					...
					>
						<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="size-3.5" aria-hidden="true">
							<path stroke-linecap="round" stroke-linejoin="round" d="M6 6h4.5v4.5H6V6Zm7.5 0H18v4.5h-4.5V6ZM6 13.5h4.5V18H6v-4.5Zm4.5-5.25h3m-3 7.5h3m0-7.5v7.5m0 0H18" />
						</svg>
						<span class="truncate">企業工作流中心</span>
					</a>
					{#if shiftKey}
						<Tooltip content={isPinned('workflows') ? $i18n.t('Unpin from Sidebar') : $i18n.t('Pin to Sidebar')}>
							<button type="button" class="mr-1 rounded-lg p-1 transition hover:bg-gray-100/60 dark:hover:bg-gray-700/60" on:click|preventDefault|stopPropagation={() => togglePin('workflows')}>
